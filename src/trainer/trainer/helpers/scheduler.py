...
Original file: src/trainer/trainer/helpers/scheduler.py
Author: naoto0804
License: Apache-2.0 License
"""

# LR scheduler with warmup borrowed from
# https://github.com/cientgu/VQ-Diffusion/blob/main/image_synthesis/engine/lr_scheduler.py
...
def _d3pm_func(epoch: int):
    warmup_epoch = 10
    if epoch <= warmup_epoch:
        mult = 1.0
    else:
        mult = 1 / math.sqrt(epoch - warmup_epoch)
    return mult


D3PMScheduler = partial(
    torch.optim.lr_scheduler.LambdaLR, lr_lambda=lambda epoch: _d3pm_func(epoch)
)
