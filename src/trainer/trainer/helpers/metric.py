...
Original file: src/trainer/trainer/helpers/metric.py
Author: naoto0804
...
def compute_generative_model_scores(
    ...
    try:
        ...
        print('Error in calculating FID')
        print(e)
        results["fid"] = -1.0
...
def compute_overlap(bbox: FloatTensor, mask: BoolTensor) -> Dict[str, FloatTensor]:
    ...
    """
    B, S = mask.size() # mask: valid object or not [B(batch_size), S(max_element)]
    bbox = bbox.masked_fill(~mask.unsqueeze(-1), 0)
    bbox = bbox.permute(2, 0, 1) # [B, max_element, 4] -> [4, B, S]

    l1, t1, r1, b1 = convert_xywh_to_ltrb(bbox.unsqueeze(-1)) # [B, S, 1] x4
    l2, t2, r2, b2 = convert_xywh_to_ltrb(bbox.unsqueeze(-2)) # [B, 1, S] x4
    a1 = (r1 - l1) * (b1 - t1)

    # intersection
    l_max = torch.maximum(l1, l2) # [B, S, S]
    r_min = torch.minimum(r1, r2) # [B, S, S]
    t_max = torch.maximum(t1, t2) # [B, S, S]
    b_min = torch.minimum(b1, b2) # [B, S, S]
