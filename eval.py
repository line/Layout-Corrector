...
def main():
    ...
    pkl_paths.sort()
    # workaround for blt_pad
    if pkl_paths[0].split("/")[-3].startswith("blt_eccv2022_pad_kmeans_unconditional_"):
        return
    ...
    if test_cfg.get("use_train_data", False):
        split_main, split_sub = "train", "val"
    ...
    for i, batch in enumerate(main_dataloader):
        ...
        if args.compute_real:
            ...
            label_names = main_dataset.labels
            for i, l in enumerate(label_names):
                label_mask = ((label == i) & mask).cpu()
                for k, v in compute_alignment(bbox.cpu(), label_mask).items():
                    batch_metrics[f'{i}_{l}_{k}'] += v.sum().item()
                for k, v in compute_overlap(bbox.cpu(), label_mask).items():
                    batch_metrics[f'{i}_{l}_{k}'] += v.sum().item()
    ...
    for pkl_path in pkl_paths:
        ...
        seed = x['train_cfg']['seed']
        scores_all['seed'].append(seed)

        if 't_total' in x: # record inference time
            scores_all['t_total'].append(x['t_total'])
        if 'N_total' in x: # record num_samples
            scores_all['N_total'].append(x['N_total'])
        ...
        for i in range(0, len(generated), args.batch_size):
            ...
            # label_names = main_dataset.labels
            # for i, l in enumerate(label_names):
            #     label_mask = ((label == i) & mask)
            #     for k, v in compute_alignment(bbox, label_mask).items():
            #         batch_metrics[f'{i}_{l}_{k}'] += v.sum().item()
            #     for k, v in compute_overlap(bbox, label_mask).items():
            #         batch_metrics[f'{i}_{l}_{k}'] += v.sum().item()
    ...
    seed_list = scores_all['seed']

    for i, seed_num in enumerate(seed_list):
        _scores_fake = {k: v[i] for k, v in scores_all.items()}
        with open(os.path.join(args.result_dir, f'scores_fake_seed_{seed_num}.json'), 'w') as f:
            json.dump(_scores_fake, f, ensure_ascii=False, indent=4, sort_keys=False, separators=(',', ': '))

    if len(seed_list) > 1:
        _scores_avg = {k: np.array(v).mean() for k, v in scores_all.items()}
        _scores_avg['seed'] = '_'.join([str(s) for s in seed_list]) # "{seed1}_{seed2}_{seed3}"
        with open(os.path.join(args.result_dir, f'scores_fake_avg.json'), 'w') as f:
            json.dump(_scores_avg, f, ensure_ascii=False, indent=4, sort_keys=False, separators=(',', ': '))

    if args.compute_real:
        scores_real = {k: v[0] for k, v in scores_real.items()}
        with open(os.path.join(args.result_dir, 'scores_real.json'), 'w') as f:
            json.dump(scores_real, f, ensure_ascii=False, indent=4, sort_keys=False, separators=(',', ': '))
